...
The visible language control is a shared accessible Radix Select popup, not a browser-native menu; it changes presentation only and preserves that cookie/provider boundary. Dashboard charts and data art remain presentation-only: Recharts is composed through the shared UI chart primitive, while decorative points are deterministic and reduced-motion-safe.

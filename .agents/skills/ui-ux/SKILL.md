...
2. Preserve the current information architecture, HSK wallet/session boundaries, transaction states, loading/error/empty/partial states, live event-derived analytics, and technical literals. A sponsored claim must show explicit beneficiary confirmation, the gas payer, request/receipt status, and the always-available manual fallback.

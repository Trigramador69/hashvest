...
export async function generateAbis() {
  const exports = await Promise.all(contracts.map(async ([name, exportName]) => {
    const artifact = JSON.parse(await readFile(artifactPath(name), "utf8"));
    if (!Array.isArray(artifact.abi)) throw new Error(`Missing ABI for ${name}`);
    return `export const ${exportName} = ${JSON.stringify(artifact.abi)} as const;`;
  }));
  return format(`// Generated from Foundry artifacts by pnpm contracts:sync. Do not edit.\n${exports.join("\n\n")}\n`, { parser: "typescript" });
}
...
export function deploymentFromBroadcast(broadcast) {
  ...
  for (const [name, field] of [["HashVestFactory", "factory"], ["DemoToken", "demoToken"], ["DemoEligibilityProvider", "eligibilityProvider"]]) {
    const matches = broadcast.transactions?.filter((tx) => tx.contractName === name && tx.transactionType === "CREATE");
    if (matches?.length !== 1) throw new Error(`Expected one ${name} deployment`);
    const tx = matches[0];
    if (!isHash(tx.hash) || !isAddress(tx.contractAddress) || tx.contractAddress === zeroAddress) throw new Error(`Invalid ${name} deployment data`);
    if (tx.transaction?.chainId != null && Number(tx.transaction.chainId) !== 133) throw new Error(`Wrong-chain transaction for ${name}`);
    const receipt = broadcast.receipts?.find((entry) => entry.transactionHash?.toLowerCase() === tx.hash.toLowerCase());
    if (!receipt || !(Number(receipt.status) === 1 || receipt.status === "success") || receipt.contractAddress?.toLowerCase() !== tx.contractAddress.toLowerCase()) throw new Error(`Missing successful ${name} receipt`);
    if (!isAddress(tx.transaction.from)) throw new Error("Missing deployer");
    const sender = getAddress(tx.transaction.from);
    if (deployer && sender !== deployer) throw new Error("Inconsistent deployer");
    deployer = sender;
...
async function main() {
  ...
  if (mode && mode !== "--check" && mode !== "--addresses") throw new Error("Usage: sync-contracts.mjs [--check|--addresses]");
  if (mode === "--addresses") {
    const deployment = deploymentFromBroadcast(JSON.parse(await readFile(broadcastPath, "utf8")));
    await writeFile(deploymentPath, await format(JSON.stringify(deployment), { parser: "json" }));
  ...
  if (mode === "--check") {
    if ((await readFile(target, "utf8")) !== output) throw new Error("Generated ABIs are stale. Run pnpm contracts:sync.");

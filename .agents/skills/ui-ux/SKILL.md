...
2. Preserve the product information architecture: `/app` is the live overview, `/app/grants` is the direct Issued/Received/Review surface, and `/app/settings` owns organization context. Preserve HSK wallet/session boundaries, transaction states, loading/error/empty/partial states, live event-derived analytics, and technical literals. Legacy paths may redirect but must not render duplicate UI. A sponsored claim must show explicit beneficiary confirmation, the gas payer, request/receipt status, and the always-available manual fallback.
...
4. Reuse existing components and translation keys. Every new user-visible string must be added to the typed English source dictionary and translated for ES and zh-CN; keep navigation limited to working product surfaces and remove dead controls. Use the [`localization`](../localization/SKILL.md) skill as well.

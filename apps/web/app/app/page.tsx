  ...
  return (
    <Panel className="overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-soft px-5 py-4">
        ...
        >
          {t("orgs.create")}{" "}
          <ArrowUpRight className="inline size-3" strokeWidth={1.25} />
        </Link>
      </div>
      <div className="grid gap-px bg-border-soft sm:grid-cols-2 lg:grid-cols-3">
        {organizations.data.slice(0, 3).map((organization) => (
          <Link
            ...
            className="bg-surface-1 p-4 transition-colors hover:bg-surface-hover"
          >
...
function DirectGrants({
  ...
  return (
    <Panel>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-soft px-5 py-4">
        <div>
          <h2 className="font-mono text-[15px] font-medium text-foreground">
            {t("dashboard.direct.title")}
          </h2>
          <p className="mt-1 text-xs text-muted-foreground">
            {t("dashboard.direct.lede")}
          </p>
        </div>
        <Link href="/grants/new" className={buttonVariants({ size: "sm" })}>
          <Plus className="size-3" strokeWidth={1.25} />{" "}
          {t("dashboard.createGrant")}
        </Link>
      </div>
      <div
        className="flex gap-1 border-b border-border-soft px-5 pt-3"
        role="tablist"
  ...
  return (
    <div className="space-y-5">
      <section className="relative grid min-h-[195px] items-center overflow-hidden lg:grid-cols-8">
        ...
        />
        <div className="absolute right-0 top-6 hidden whitespace-pre-line font-mono text-[10px] leading-6 text-muted-foreground/60 sm:block">
          {t("dashboard.hero.rail")}
        </div>
      </section>
      <NetworkNotice />
      <DashboardOverview

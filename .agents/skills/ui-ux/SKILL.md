...
3. Prefer semantic HTML, keyboard access, visible focus, useful labels, readable contrast, reduced-motion behavior, and responsive behavior. Native checkbox, radio, and range controls inherit `accent-color` from `--accent-green`; do not leave the browser-default blue. Build reusable primitives (`Panel`, `MetricCard`, `DataArt`) instead of duplicating visual rules.

...
3. Prefer semantic HTML, keyboard access, visible focus, useful labels, readable contrast, reduced-motion behavior, and responsive behavior. Native checkbox, radio, and range controls inherit `accent-color` from `--accent-green`; do not leave the browser-default blue. Build reusable primitives (`Panel`, `MetricCard`, `DataArt`, `ChartContainer`, and `Select`) instead of duplicating visual rules. Compose dashboard charts through the Recharts-backed chart primitive and keep decorative point art deterministic and sparse.

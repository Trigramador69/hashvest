...
The visible language and grant/workspace choice controls are shared accessible
Radix Select popups, not browser-native menus; the language control changes
presentation only and preserves that cookie/provider boundary. Destructive
actions and expandable panels use shared Radix-backed AlertDialog, Dialog, and
Collapsible primitives, so product UI does not fall back to browser alerts,
confirms, native selects, or details widgets. Dashboard charts and data art
remain presentation-only: Recharts is composed through the shared UI chart
primitive, while decorative points are deterministic and reduced-motion-safe.

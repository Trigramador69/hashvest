...
3. Keep locale selection and fallback behavior consistent with the existing cookie/provider/server boundary. The shared language control is an accessible Radix Select popup; preserve its keyboard/focus behavior, cookie write, immediate `document.documentElement.lang` update, and `router.refresh()` without changing unrelated native form selects. If adding a language rather than filling an existing one, update the locale type, registry, dictionary exports, fallback tests, and documentation together.
4. Replace user-facing literals at the relevant surface, including landing, workspace, switcher, members, queues, wizard, templates, grant detail, actions, status, funding, and business-model messages when the issue includes them. Keep authenticated shell labels limited to live Overview, Grants, Organizations, and Settings surfaces; the public `/plans` link and product-model copy are a separate presentation path. Organization copy belongs to the `/app/organizations` routes. For sponsored actions, cover owner policy controls, member usage/limits, actor confirmation for claims and reviews, relayer/gas-payer status, request/receipt states, failures, and the manual fallback in every selected locale. For milestone evidence, cover title, loading, error, retry, empty, unsafe-link, submitted status, submitter, created/updated timestamps, note, evidence types, and the review-queue title in every selected locale.

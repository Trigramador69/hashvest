...
- The app shell exposes Overview, Grants, Organizations, and Settings. Organizations live under `/app/organizations`; workspace session, language, network, and links to each organization's sponsored-claim policy live under `/app/settings`; direct role-based grants live under `/app/grants`. Keep `/app/settings/organizations/...` as redirects only and do not add controls without a real destination or backing behavior.
- The language and form-choice controls are accessible custom popups backed by shared Radix Select primitives; product confirmations and expandable sections use the shared `AlertDialog`, `Dialog`, and `Collapsible` primitives instead of browser alerts, confirms, native selects, or native details. Dashboard charts stay read-only Recharts projections through `apps/web/components/ui/chart.tsx`, and `DataArt` remains sparse deterministic point texture with reduced-motion-safe entrance effects.
...
- AI drafts; humans decide. A model may never sign, fund, approve, claim, revoke, arbitrate, or select a wallet, and its output reaches the wizard only as an editable preset that passes the same validation a hand-written one does. Prompts and raw model output are never retained. Only template configuration explicitly reviewed and saved by an owner may persist through ordinary template CRUD. Provider secrets are server-only and read in one allowlisted module. See [`docs/ai-grant-builder.md`](docs/ai-grant-builder.md) and [`docs/ai-tools.md`](docs/ai-tools.md).
- AI tools use independent collapsible sections in the Template step, new-template editor, private milestone evidence and the organization report. Every analysis requires organization membership, reads HSK on the server rather than trusting browser-supplied figures, cites only sources the server supplied, and never downloads evidence links or executes recommendations. The report summary additionally obeys the reporting rule above: no cross-token total, price, conversion, TVL or yield, and a vault that could not be read or fell outside the read limit is declared, never counted as zero. Manual creation, review and reporting all remain available on provider failure.
...
- Organization-created grants use `HashVestFactory.createSponsoredGrant` and `SponsoredGrantVault` for beneficiary-signed claims and reviewer-signed milestone approvals submitted by a gas relayer; direct and pre-existing grants retain the wallet-paid `claim()` / `approveMilestone()` fallbacks. The Cloud may track policy, budgets, allowlists, and request/receipt state, but HSK remains authoritative.
...
- Organization reporting and lifecycle notifications are derived from live GrantVault reads, never stored as authority. Amounts are summed only within one ERC20 contract: no cross-token total, price, conversion, TVL, or yield. A notification is identified by the state fact it reports, never by the clock, which is what keeps it deduplicated and bounded without an indexer. Only read/unread is persisted, scoped to the session wallet. A failed vault read must stay visibly distinct from a confirmed one. See [`docs/organization-reporting.md`](docs/organization-reporting.md).
...
- Browser checks must start this checkout's server; do not reuse a server from another worktree. Set `HASHVEST_VISUAL_PORT` when the default 3100 is occupied. Optional `HASHVEST_LIVE_AI=1` smoke uses synthetic data only and is not part of offline CI.

...
- The language control is an accessible custom popup backed by the shared locale cookie/provider boundary; dashboard charts stay read-only Recharts projections through `apps/web/components/ui/chart.tsx`, and `DataArt` remains sparse deterministic point texture with reduced-motion-safe entrance effects.
